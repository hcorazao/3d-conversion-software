 ...
 * Radius condition
 */
export default class VertexRadiusAndAttributeCondition implements IVertexCondition {
  /**
   ...
   */
  constructor(center: Vector3, radius: number, distanceMap: Map<number, number>, vertexAttributes: Uint8Array, inactiveMask: number) {
    this.center = center;
    this.radius = radius;
    this.distanceMap = distanceMap;
    this.vertexAttributes = vertexAttributes;
    this.inactiveMask = inactiveMask;
  }
  ...
  private vertexAttributes: Uint8Array;

  private inactiveMask: number;
  ...
  evaluateConditionXYZ(x: number, y: number, z: number, idx: number): boolean {
    if ((this.vertexAttributes[idx] & this.inactiveMask) === this.inactiveMask) {
      return false;
    }
